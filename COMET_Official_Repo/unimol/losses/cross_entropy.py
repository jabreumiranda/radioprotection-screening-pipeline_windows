...
# loss function is selected based on loss_func arg of the script

@register_loss("np_finetune_cross_entropy")
class NPFinetuneCrossEntropyLoss(CrossEntropyLoss):
    ...
    def forward(self, model, sample, reduce=True, target_subclass="finetune_target"):
        ...
        """
        # print(self.args.classification_head_name, "NPFinetuneCrossEntropyLoss, sample: ", sample)
        net_output = model(
            sample,
            # **sample["net_input"],
            features_only=True,
            np_classification_head_name=self.args.classification_head_name,
        )
        # net_output = model(
        #     **sample["net_input"],
        #     features_only=True,
        ...
        # print("NPFinetuneCrossEntropyLoss loss: ", loss)
        ...
        if not self.training:
            ...
            logging_output = {
                "loss": loss.data,
                "prob": probs.data,
                "target": sample["target"][target_subclass].view(-1).data,
    ...
    def compute_loss(self, model, net_output, sample, reduce=True, target_subclass="finetune_target"):
        ...
        targets = sample["target"][target_subclass].view(-1)
        loss = F.nll_loss(
            lprobs,
            targets,
...
class MultiTaskBCELoss(CrossEntropyLoss):
    ...
    def forward(self, model, sample, reduce=True, target_subclass="finetune_target"):
        ...
        logit_output = net_output[0]
        is_valid = sample["target"][target_subclass] > -0.5
    ...
    def compute_loss(self, model, net_output, sample, reduce=True, is_valid=None, target_subclass="finetune_target"):
        pred = net_output[is_valid].float()
        targets = sample["target"][target_subclass][is_valid].float()
