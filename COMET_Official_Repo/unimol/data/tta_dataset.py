...
class TTADataset(BaseWrapperDataset):
    ...
    def __len__(self):
        return len(self.dataset) * self.conf_size # number of mol X conformation size
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        coordinates = np.array(self.dataset[smi_idx][self.coordinates][coord_idx]) # remove stochasticity in sampling coord: samples all possible coords
        smi = self.dataset[smi_idx]["smi"]
        if "target" in self.dataset[smi_idx]:
