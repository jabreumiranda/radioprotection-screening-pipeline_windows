...
class ConformerSampleDataset(BaseWrapperDataset):
    ...
    def __cached_item__(self, index: int, epoch: int, conf_ind: int=None):
        # print("self.dataset: ", self.dataset)
        # print("index: ", index)
        # print("self.atoms: ", self.atoms)
        # print("before atoms = np.array(self.dataset[index][self.atoms])")
        atoms = np.array(self.dataset[index][self.atoms])
        # print("atoms: ", atoms)
        # print("after atoms = np.array(self.dataset[index][self.atoms])")
        assert len(atoms) > 0
        size = len(self.dataset[index][self.coordinates])
        if conf_ind is not None:
            sample_idx = conf_ind
        else:
            with data_utils.numpy_seed(self.seed, epoch, index): # randomly sample a coord from a list of possible conformations with size conf_size
                sample_idx = np.random.randint(size)
        coordinates = self.dataset[index][self.coordinates][sample_idx]
        # print("__cached_item__ coordinates.shape: ", coordinates.shape)
        return {"atoms": atoms, "coordinates": coordinates.astype(np.float32)}

    def __getitem__(self, index: int, conf_ind: int=None):
        return self.__cached_item__(index, self.epoch, conf_ind)
