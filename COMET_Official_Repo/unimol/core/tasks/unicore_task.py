...
import torch
from unimol.core import metrics, utils
from unimol.core.data import UnicoreDataset, data_utils, iterators
...
class UnicoreTask(object):
    ...
    def setup_task(cls, args: Namespace, **kwargs):
        # print("setup_task in unicore_task.py, cls: ", cls)
    ...
    def dataset(self, split):
        ...
        """
        from unimol.core.data import UnicoreDataset
    ...
    def build_loss(self, args: Namespace):
        ...
        """
        from unimol.core import losses
    ...
    def train_step(
        ...
        """
        # print("train_step in unicore unicore_task.py") # used by unimol_finetune.py
        # raise
        model.train()
        model.set_num_updates(update_num)
        with torch.autograd.profiler.record_function("forward"):
            # task_schema added to loss forward
            loss, sample_size, logging_output = loss(model, sample, task_schema=self.task_schema, loss_sample_dropout=self.args.loss_sample_dropout, multitask_reg=self.args.multitask_reg, cagrad_c=self.args.cagrad_c, contrast_margin_coeff=self.args.contrast_margin_coeff)
        if ignore_grad:
            loss *= 0
        # if debug:
        ...
        #         try:
        #             param_grad = torch.autograd.grad(loss, param, retain_graph=True, allow_unused=True)
        #             print("param name: ", name, "param_grad: ", param_grad)
        #         except:
        #             print("param name causing param_grad error: ", name)
        #     for prop in self.np_prop_schema:
        #         emb_grad = torch.autograd.grad(loss, model.np_prop_embs[prop].parameters(), retain_graph=True, allow_unused=True)
        #         print("prop: ", prop, "emb_grad: ", emb_grad)
    ...
    def valid_step(self, sample, model, loss, test=False, infer=False, output_cls_rep=False):
        # print("valid_step in unicore unicore_task.py") # used by unimol_finetune.py and unimol's infer.py
        model.eval()
        with torch.no_grad():
            # print("valid_step self.task_schema: ", self.task_schema)
            loss, sample_size, logging_output = loss(model, sample, task_schema=self.task_schema, contrast_margin_coeff=self.args.contrast_margin_coeff, 
                                                     infer=infer, output_cls_rep=output_cls_rep)
        return loss, sample_size, logging_output
    ...
    def reduce_metrics(self, logging_outputs, loss, split='train', infer=False):
        ...
        reduced_metrics_dict = loss.__class__.reduce_metrics(logging_outputs, split, infer=infer)
