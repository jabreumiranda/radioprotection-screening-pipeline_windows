...
import torch
from unimol.core import checkpoint_utils, models, optim, utils
from unimol.core.distributed import utils as distributed_utils
from unimol.core.logging import meters, metrics
from unimol.core.nan_detector import NanDetector
from unimol.core.optim import lr_scheduler
from unimol.core.utils import tensor_tree_map
...
class Trainer(object):
    ...
    def state_dict(self):
        # print("Trainer state_dict, self.model.state_dict(): ", self.model.state_dict())
        state_dict = {
            "args": self.args,
    ...
    def save_checkpoint(self, filename, extra_state):
        ...
        # print("before state_dict = utils.move_to_cpu(self.state_dict())")
        state_dict = utils.move_to_cpu(self.state_dict())
        # print("after state_dict = utils.move_to_cpu(self.state_dict())")
    ...
    def load_checkpoint(
        ...
        reset_meters=False,
        load_full_np_model=False, # whether to load only mol model or full LNP model if LNP model is used
    ):
        """
        ...
        if bexists:
            ...
            # print("NP in self.model.__class__.__name__:", "NP" in self.model.__class__.__name__)
            # ! Load checkpoint of mol_model, not NP model
            if "NP" in self.model.__class__.__name__:
                # mol_model = self.model.mol_model
                if load_full_np_model:
                    model_to_load = self.model # load weights of whole LNP model
                else:
                    model_to_load = self.model.mol_model # load only mol_model
            else:
            ...
            try:
                if self.args.load_from_ema:
                    ...
                    errors = model_to_load.load_state_dict(
                ...
                # print("loaded checkpoint for mol_model!")
    ...
    def get_train_iterator(
        ...
        # Dynamically change dataset sources (e.g. dropping datasets that are overfitted by model)
        # Use this to change dataset sources, by dropping data subset
        if load_dataset:
            logger.info("loading train data for epoch {}".format(epoch))
            if self.args.concat_datasets:
                print("get_train_iterator, loading dataset dropped_datasets: ", dropped_datasets)
                self.task.load_concat_dataset(
        ...
        # Dynamically change dataset sources (e.g. dropping datasets that are overfitted by model)
        # print("get_train_iterator, self.task.dataset: ", self.task.dataset)
        batch_iterator = self.task.get_batch_iterator(
            dataset=self.task.dataset(self.args.train_subset), # Changed "self.task.dataset(self.args.train_subset)" to the new concatenated dataset that excludes dropped data subsets
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        # print("train_step in unicore trainer.py") # used by unimol_finetune.py
        self.model.train()
        self.loss.train()
        self.zero_grad()
            ...
            try:
                with maybe_no_sync():
                    ...
                    with utils.torch_seed(
                        ...
                        # print("before loss, sample_size_i, logging_output = self.task.train_step")
                        ...
                        # print("after loss, sample_size_i, logging_output = self.task.train_step")
                        del loss
                    if self.args.per_sample_clip_norm > 0:
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        # print("valid_step in unicore trainer.py") # used by unimol_finetune.py
        ...
        with torch.no_grad():
            ...
            try:
                # print("before _loss, sample_size, logging_output = self.task.valid_step")
                _loss, sample_size, logging_output = self.task.valid_step(
                    sample, self.model, self.loss
                )
                # print("after _loss, sample_size, logging_output = self.task.valid_step")
            except RuntimeError as e:
                if "out of memory" in str(e):
                    self._log_oom(e)
                    if not raise_oom:
        ...
        # print("valid_step logging_outputs: ", logging_outputs)
