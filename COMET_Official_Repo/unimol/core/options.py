...
from unimol.core.utils import csv_str_list, eval_bool, eval_str_dict, eval_str_list, import_user_module  # noqa
...
def parse_args_and_arch(
    ...
    from unimol.core.models import ARCH_MODEL_REGISTRY, ARCH_CONFIG_REGISTRY, MODEL_REGISTRY
    ...
    if hasattr(args, "task"):
        from unimol.core.tasks import TASK_REGISTRY

        TASK_REGISTRY[args.task].add_args(parser)

    # Add *-specific args to parser.
    from unimol.core.registry import REGISTRIES
...
def add_dataset_args(parser, train=False, gen=False):
    ...
    #new arg with np train/infer
    group.add_argument('--train-data-ratio', default=1.0, type=float,
                        help='ratio of training data to use, 1.0 means all. Use this to easily study the effect of data size on performance')
    # fmt: on
    return group
...
def add_checkpoint_args(parser):
    ...
    group.add_argument('--best-checkpoint-metric', type=str, default='loss',
                       help='metric to use for saving "best" checkpoints and to determine when to stop training, if name exist in multiple metrics, the mean of them will be used')
    ...
    # TODO NOW: Dynamically change dataset sources (e.g. dropping datasets that are overfitted by model) by tracking overfitting with these metrics 
    group.add_argument('--metrics-to-dropped-datasets', type=str, default=None,
                       help='metric to use for tracking overfitting and dropping datasets')
    group.add_argument('--maximize-metrics-that-drop-datasets', action='store_true',
                       help='don\'t save optimizer-state as part of checkpoint')
    group.add_argument('--subdataset-patience', type=int, default=-1, metavar='N',
                       help="early stop training if valid performance doesn't "
                            "improve for N consecutive validation runs for subdataset; note "
                            "that this is influenced by --validate-interval")
    group.add_argument('--start-epoch-to-drop-datasets', type=int, default=0, metavar='N',
                       help='Epoch to start dropping datasets that are overfitted by model, evaluated through valid loss')
    group.add_argument('--leave-last-subdataset-to-train', action='store_true',
                       help='Leave last subdataset in training, even after patience is reached')
    # fmt: on
    return group
...
def add_model_args(parser):
    ...
    from unimol.core.models import ARCH_MODEL_REGISTRY
