...
import torch
from unimol.core.data import Dictionary
from functools import lru_cache
...
class TokenizeDataset(BaseWrapperDataset):
    ...
    def __getitem__(self, index: int):
        ...
        # print("TokenizeDataset __getitem__ raw_data: ", raw_data) # list of tokens
        if hasattr(raw_data, '__len__'):
            assert len(raw_data) < self.max_seq_len and len(raw_data) > 0
        if isinstance(raw_data, float) or isinstance(raw_data, float):
