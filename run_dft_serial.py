...
# 1. Reescrever os .inp no modo estrito serial (1 core, sem mpiexec)
for inp_file in glob.glob(os.path.join(orca_dir, "*_neutral_opt.inp")):
    with open(inp_file, 'r', encoding='utf-8') as f:
        lines = f.readlines()
    
    clean_lines = []
    for line in lines:
        if "%pal" in line or "nprocs" in line or "PAL" in line:
            continue
        if line.startswith("!"):
            # Garante sintaxe limpa B3LYP com resolucao de identidade def2/J e dispersao D3BJ
            clean_lines.append("! B3LYP def2-SVP def2/J D3BJ Opt Freq CPCM(Water)\n")
        else:
...
# 2. Limpar logs e temporarios anteriores
for tmp in glob.glob(os.path.join(orca_dir, "*tmp*")) + glob.glob(os.path.join(orca_dir, "*.out")):
    try:
        os.remove(tmp)
    except Exception:
        pass
...
print(f"Iniciando execucao serial no ORCA para {len(inp_files)} moleculas...\n")
...
for inp_path in inp_files:
    ...
    print(f"-> [ORCA DFT] Calculando: {cid} (aguarde a convergencia)...")
    
    cmd = f'"{orca_bin}" "{inp_path}"'
    with open(out_path, "w", encoding="utf-8") as out_f:
        subprocess.run(cmd, shell=True, stdout=out_f, stderr=subprocess.STDOUT, cwd=orca_dir)
