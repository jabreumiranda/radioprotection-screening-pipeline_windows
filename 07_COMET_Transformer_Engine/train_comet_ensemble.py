...
# 1. Carregar Dados e Embeddings
X_data = np.load(os.path.join(base_dir, 'comet_embeddings_248.npy'))
...
# Converter afinidade para valor positivo de forca de ligacao (mais alto = melhor)
Y_data[:, 0] = np.abs(Y_data[:, 0])

# ==============================================================================
# DEFINICAO DA ARQUITETURA TRANSFORMER COMET
# ==============================================================================
...
# ==============================================================================
# FUNCAO DE PERDA: PAIRWISE RANKING LOSS COM MARGEM
# ==============================================================================
...
# ==============================================================================
# TREINAMENTO DO ENSEMBLE 5-FOLD DO COMET
# ==============================================================================
print("=" * 70)
print("INICIANDO TREINAMENTO DO ENSEMBLE DE 5 MODELOS COMET (CROSS-VALIDATION)")
print("=" * 70)
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X_data)):
    ...
    model = COMETTransformer(input_dim=X_data.shape[1], embed_dim=256, n_heads=4, num_layers=2, num_tasks=4)
    ...
    for epoch in range(epochs):
        optimizer.zero_grad()
        # Noise Augmentation (10% no embedding de entrada)
    ...
    # Calculo de Spearman Rank Correlation por tarefa no conjunto de validacao
    spearman_scores = []
    pearson_scores = []
    for t in range(4):
        sr, _ = spearmanr(val_preds[:, t], val_targets[:, t])
        pr, _ = pearsonr(val_preds[:, t], val_targets[:, t])
        spearman_scores.append(sr)
        pearson_scores.append(pr)
        
    print(f"   Fold {fold+1} Validacao:")
    ...
    torch.save(model.state_dict(), os.path.join(models_dir, f'comet_fold_{fold+1}.pt'))

mean_spearman = np.mean(ensemble_metrics, axis=0)

print("\n" + "=" * 70)
print("PERFORMANCE MEDIA DO ENSEMBLE COMET (5-FOLD CROSS-VALIDATION):")
...
print(f"Modelos salvos com sucesso em: {models_dir}")
print("=" * 70 + "\n")
