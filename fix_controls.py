...
# Controles com SMILES canonicos corrigidos
gold_standards = [
    ("REF_HOECHST33258", "Cc1ccc2[nH]c(nc2c1)-c1ccc2[nH]c(nc2c1)-c1ccc(N2CCNCC2)cc1", "Reference: DNA Minor Groove Standard"),
    ...
    ("REF_EGCG", "O=C(Oc1cc(O)c(O)c(O)c1)C1CC2=C(O)C=C(O)C=C2OC1c1cc(O)c(O)c(O)c1", "Reference: Polyphenolic Scavenger"),
...
for cid, sm, cat in gold_standards:
    ...
    # Gerar input ORCA para o controle
...
*
"""
    with open(os.path.join(orca_dir, f"{cid}_neutral_opt.inp"), 'w') as f:
        f.write(orca_input)
...
shortlist = pd.concat([top_aim3, top_aim2, top_aim1, df_refs], ignore_index=True)
...
print("\nKekulizacao corrigida e Controles 100% integrados!")
print(df_refs[['ChEMBL_ID', 'MW_Da', 'DeltaG_kcal_mol', 'RSI_Score', 'DARS_Score']])
